...
public class UserService : IUserService
{
    ...
    public async Task<AuthResponse> RegisterAsync(RegisterRequest request, int orgId, int roleId)
    {
        ...
        try
        {
            ...
            // Persist category allowlist - meaningful for 'User' AND 'Admin'
            // accounts now (was 'User'-only; a SuperAdmin creating an Admin
            // account can now restrict that Admin's categories too, same
            // checkbox UI the create-user form already offers for 'User').
            // SuperAdmin accounts never get one - always unrestricted.
            if ((user.UserTypeId == UserTypeIds.User || user.UserTypeId == UserTypeIds.Admin)
                && request.AllowedCategories is { Length: > 0 })
            {
    ...
    public async Task<AuthResponse> UpdateUserAsync(UpdateUserRequest request)
    {
        ...
        try
        {
            ...
            // 'User' and 'Admin' now (was 'User'-only) - see RegisterAsync's
            // matching check for why. Unlike RegisterAsync this has no
            // "is { Length: > 0 }" guard - an empty/null AllowedCategories
            // here is a deliberate "clear the restriction back to
            // unrestricted" edit, not "nothing was submitted", so it's
            // written through as-is either way.
            if (currentUser.UserTypeId == UserTypeIds.User || currentUser.UserTypeId == UserTypeIds.Admin)
            {
